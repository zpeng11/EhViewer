...
fun DownloadInfo.findLocalThumbFile(): Path? = downloadDir?.let { dir ->
    dir.findImmediateSidecarThumbFile()?.let { return it }
    if (dir.findAnySidecarThumbFile() != null) {
        null
    } else {
        findCachedThumbFile(gid)
    }
} ?: findCachedThumbFile(gid)
...
private fun DownloadInfo.buildThumbPath(dir: Path, extension: String) = if (dir.isPhysicalPath) {
    dir / "thumb.${extension.lowercase()}"
} else {
    buildCachedThumbPath(gid, extension)
}
...
private suspend fun DownloadInfo.writeStaticThumbFromSpiderDen(spiderDen: SpiderDen, index: Int, target: Path) = runCatching {
    spiderDen.getImageSource(index).use { source ->
...
suspend fun DownloadInfo.ensureLocalThumbFile(): Path? {
    ...
    dir.findAnySidecarThumbFile()?.let { sidecar ->
        ...
        val target = buildThumbPath(dir, STATIC_THUMB_EXTENSION)
        ...
        if (staticized != null) {
            if (!dir.isPhysicalPath) pruneCachedThumbVariants(gid, staticized)
            return staticized
        }
    }

    findCachedThumbFile(gid)?.let { return it }

    val spiderDen = SpiderDen(localGalleryInfo(), localDirname)
    spiderDen.initDownloadDirIfExist()
    val localThumb = runCatching {
        thumbnailCandidateIndices().firstNotNullOfOrNull { index ->
            spiderDen.getExtension(index)?.takeIf(String::isNotBlank)?.let { extension ->
                val targetExtension = if (shouldStaticize(extension)) STATIC_THUMB_EXTENSION else extension
                val target = buildThumbPath(dir, targetExtension)
                if (target.isFile) {
                    pruneThumbVariants(gid, dir, target)
                    target
                } else {
                    val saved = if (shouldStaticize(extension)) {
                        writeStaticThumbFromSpiderDen(spiderDen, index, target)
                    } else if (spiderDen.saveToPath(index, target)) {
                        target.takeIf { it.isFile }
                    } else {
                        null
                    }
                    saved?.also { pruneThumbVariants(gid, dir, it) }
                }
            }
        }
    }.onFailure {
            ...
            useArchivePageLoader(
                ...
                thumbnailCandidateIndices(loader.size).firstNotNullOfOrNull { index ->
                    ...
                    if (target.isFile) {
                        ...
                        val saved = if (shouldStaticize(extension)) {
                            writeStaticThumbFromArchive(loader, gid, index, extension, target)
