...
- [x] `S1-FAV-05` 收藏项可视化增强：在“本地收藏夹”视图中，对已归属额外收藏夹的条目展示额外收藏夹代号（1-9）徽标，帮助用户定位目标夹
  文件域：
...
| 2026-03-05 22:43 | Step 1 | S1-FAV-05 | ~ | agent | 启动收藏项可视化增强：计划仅在“本地收藏夹”视图为已归属额外收藏夹的条目显示 1..9 代号徽标，并同步覆盖列表/网格两种卡片样式 | 进行中 | 本步先完成 UI 实现与构建安装，再等待用户实机验收后决定是否标记 x |
| 2026-03-05 22:56 | Step 1 | S1-FAV-05 | ! | agent | 已完成本地收藏夹视图的额外收藏夹代号徽标实现：列表/网格卡片仅在“本地收藏夹”视图显示 1..9 数字徽标；`FavoritesScreen` 按当前收藏夹路由透传徽标 slot；补充中英文文案资源 | `:app:compileDefaultDebugKotlin` 成功，`:app:assembleDebug` 成功；按 `docs/wsl-local-build-install.md` 执行 `prepareLibraryDefinitionsDefaultRelease/MarshmallowRelease --rerun -Prelease` 与 `assembleRelease -Prelease` 成功；ADB `connect 192.168.2.93:43091` 失败（`No route to host`），`adb devices -l` 为空 | 当前阻塞于实机连接不可达，待用户恢复无线 ADB 目标或提供新的设备地址后再继续安装与验收 |
| 2026-03-05 23:05 | Step 1 | S1-FAV-05 | ~ | agent | 重新提升权限后恢复 ADB 链路：重启本机 ADB server 后成功连接 `192.168.2.93:43091`，并按 `docs/wsl-local-build-install.md` 安装 default universal release；本地收藏夹视图的额外收藏夹徽标实现已部署到实机 | `adb connect` 成功；安装时 incremental install 被拒后自动回退 streamed install 并成功；`dumpsys package` 显示 `versionName=1.14.6`、`versionCode=180063`、`lastUpdateTime=2026-03-06 12:05:21` | 等待用户实机验证 `S1-ACC-15`：仅在“本地收藏夹”视图显示 1..9 徽标，且代号与目标额外收藏夹一致 |
| 2026-03-05 23:39 | Step 1 | S1-FAV-05 | ~ | agent | 为支撑 `S1-FAV-05` 验收补充下载页入口：扩展 `FabLayout` 支持长按动作；Downloads 多选态爱心按钮短按仍加入默认本地收藏，长按则弹出现有额外收藏夹列表并直接分配；补充“选择额外收藏夹/无额外收藏夹/加入额外收藏夹成功失败”文案 | `:app:compileDefaultDebugKotlin` 成功，`:app:assembleDebug` 成功；按 `docs/wsl-local-build-install.md` 执行 `prepareLibraryDefinitionsDefaultRelease/MarshmallowRelease --rerun -Prelease` 与 `assembleRelease -Prelease` 成功；ADB 安装成功（serial=192.168.2.93:43091，versionName=1.14.6，versionCode=180063，lastUpdateTime=2026-03-06 12:39:05） | 等待用户实机验证下载页新增入口：长按多选态爱心按钮应弹出额外收藏夹选择并成功分配，短按保持默认本地收藏 |
